...
ALLOWED = (
    ...
    | BASIC_IO_SET
    | FILESYSTEM_SET
    | {"arch_prctl", "ioctl", "madvise", "mprotect", "mremap", "prctl", "readdir", "umask"}
)
...
class Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if completed.returncode != 0:
            raise unittest.SkipTest("cannot run bwrap (does it need to be setuid?)")
    ...
    def test_no_seccomp(self) -> None:
        for syscall in TRY_SYSCALLS:
            ...
            if syscall == "ioctl TIOCSTI CVE-2019-10063" and completed.returncode == errno.ENOENT:
    ...
    def test_seccomp_denylist(self) -> None:
        with tempfile.TemporaryFile() as denylist_temp:
            ...
            denylist.add_rule(
                seccomp.ERRNO(errno.ECONNREFUSED),
                "ioctl",
    ...
    def test_seccomp_stacked(self, allowlist_first=False) -> None:
        with tempfile.TemporaryFile() as allowlist_temp, tempfile.TemporaryFile() as denylist_temp:
...
def main():
    ...
    if TAPTestRunner is not None:
        ...
        if program.result.wasSuccessful():
            print(f"ok 1 - {program.result!r}")
        else:
            print(f"not ok 1 - {program.result!r}")
            sys.exit(1)
