...
def run_sbx(
    ...
    print(cmd_argv)
    cp = subprocess.run(
        argv, cwd=str(cwd), env=env, capture_output=True, timeout=TIMEOUT_SEC, text=True
    )
...
def main() -> int:
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo tempok > %TEMP%\\ws_temp_ok.txt"], WS_ROOT
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "mkdir sub && echo hi > sub\\in_sub.txt"], WS_ROOT
    )
    ...
    rc, out, err = run_sbx(
        "read-only", ["python", "-c", "open('py_should_fail.txt','w').write('x')"], WS_ROOT
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "mkdir deep\\nest && echo ok > deep\\nest\\f.txt"], WS_ROOT
    )
    add(
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo x > m1.txt & move /y m1.txt m2.txt"], WS_ROOT
    )
    add(
    ...
    if make_junction(sys_link, sys_target):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo bad > sys_link\\system32\\sbx_junc.txt"], WS_ROOT
        )
        add(
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\codex_testpipe"], WS_ROOT
    )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", "echo long > \\\\?\\C:\\sbx_longpath_test.txt"], WS_ROOT
    )
    ...
    if make_symlink(unc_link, other_to):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo unc > unc_link\\unc_test.txt"], WS_ROOT
        )
    ...
    if make_symlink(other_drive, other_target):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo drive > other_drive\\drive.txt"], WS_ROOT
        )
    ...
    rc, out, err = run_sbx(
        "workspace-write", ["cmd", "/c", f"echo leak > {outside_after_timeout}"], WS_ROOT
    )
    add(
