...
fn postgres_test_db_url() -> Option<String> {
    std::env::var("QMD_TEST_DATABASE_URL")
        .or_else(|_| std::env::var("QMD_DATABASE_URL"))
        .ok()
}

fn setup_store() -> Option<(TempDir, PostgresQmdStore, PathBuf, String)> {
    let db_url = postgres_test_db_url()?;
    ...
    let store = PostgresQmdStore::open_with_paths(db_url, &cfg_path)
        .expect("postgres integration requested via QMD_* env, but connection/setup failed");
...
fn concurrent_store_bootstrap_is_idempotent() {
    ...
    PostgresQmdStore::open_with_paths(db_url.clone(), &cfg_path)
        .expect("postgres integration requested via QMD_* env, but connection/setup failed");
