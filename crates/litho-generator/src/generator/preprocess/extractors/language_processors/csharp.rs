...
impl LanguageProcessor for CSharpProcessor {
    ...
    fn determine_component_type(&self, file_path: &Path, content: &str) -> String {
        ...
        if file_name.ends_with(".csproj") {
            ...
            } else if content.contains("<OutputType>Library</OutputType>")
                || content.contains("Microsoft.NET.Sdk")
            {
        ...
        if file_name.ends_with(".sql") {
            if content.to_uppercase().contains("CREATE TABLE")
                || content.to_uppercase().contains("ALTER TABLE")
            {
                return "sql_table_definition".to_string();
            } else if content.to_uppercase().contains("CREATE PROCEDURE")
                || content.to_uppercase().contains("ALTER PROCEDURE")
            {
                return "sql_stored_procedure".to_string();
            } else if content.to_uppercase().contains("CREATE VIEW")
                || content.to_uppercase().contains("ALTER VIEW")
            {
                return "sql_view".to_string();
            } else if content.to_uppercase().contains("CREATE FUNCTION")
                || content.to_uppercase().contains("ALTER FUNCTION")
            {
...
impl CSharpProcessor {
    ...
    fn parse_csharp_parameters(&self, params_str: &str) -> Vec<ParameterInfo> {
        ...
        for param in params_str.split(',') {
            ...
            if parts.len() >= 2 {
                let (param_type, name, is_optional) = if parts[0] == "ref"
                    || parts[0] == "out"
                    || parts[0] == "in"
                    || parts[0] == "params"
                {
    ...
    fn extract_sql_interfaces(&self, content: &str) -> Vec<InterfaceInfo> {
        ...
        let create_table_re =
            regex::Regex::new(r"(?i)CREATE\s+TABLE\s+(?:\[?(\w+)\]?\.)?\[?(\w+)\]?").unwrap();
        let alter_table_re =
            regex::Regex::new(r"(?i)ALTER\s+TABLE\s+(?:\[?(\w+)\]?\.)?\[?(\w+)\]?").unwrap();
        let create_view_re = regex::Regex::new(
            r"(?i)CREATE\s+(?:OR\s+ALTER\s+)?VIEW\s+(?:\[?(\w+)\]?\.)?\[?(\w+)\]?",
        )
        .unwrap();
        let create_proc_re = regex::Regex::new(
            r"(?i)CREATE\s+(?:OR\s+ALTER\s+)?PROC(?:EDURE)?\s+(?:\[?(\w+)\]?\.)?\[?(\w+)\]?",
        )
        .unwrap();
        let create_func_re = regex::Regex::new(
            r"(?i)CREATE\s+(?:OR\s+ALTER\s+)?FUNCTION\s+(?:\[?(\w+)\]?\.)?\[?(\w+)\]?",
        )
        .unwrap();
        let create_trigger_re = regex::Regex::new(
        ...
        let create_type_re =
            regex::Regex::new(r"(?i)CREATE\s+TYPE\s+(?:\[?(\w+)\]?\.)?\[?(\w+)\]?").unwrap();
        ...
        for line in content.lines() {
            ...
            if trimmed.contains("<Build Include=") {
                ...
                if lower.contains("tables") || lower.contains(".table.") {
                    ...
                    || lower.contains("storedprocedures")
                    || lower.contains(".proc.")
                {
                    procs += 1;
            ...
            interfaces.push(InterfaceInfo {
                ...
                parameters: vec![
                    ParameterInfo {
                        ...
                        param_type: format!("{}", tables),
                        is_optional: false,
                        description: None,
                    },
                    ParameterInfo {
                        name: "Views".to_string(),
                        param_type: format!("{}", views),
                        is_optional: false,
                        description: None,
                    },
                    ParameterInfo {
                        name: "StoredProcedures".to_string(),
                        param_type: format!("{}", procs),
                        is_optional: false,
                        description: None,
                    },
                    ParameterInfo {
                        name: "Functions".to_string(),
                        param_type: format!("{}", functions),
                        is_optional: false,
                        description: None,
                    },
                ],
    ...
    fn extract_sql_function_return_type(
        &self,
        lines: &[&str],
        start_line: usize,
    ) -> Option<String> {
