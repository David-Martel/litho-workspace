...
impl MermaidFixer {
    ...
    pub async fn fix_mermaid_charts(context: &GeneratorContext, target_dir: &Path) -> Result<()> {
        ...
        if !Self::is_available().await {
            ...
            println!("{}", msg);
            println!(
                "💡 Tip: Run 'cargo install mermaid-fixer' to install the mermaid fixing tool"
            );
            return Ok(());
        }
        ...
        println!(
            "🚀 Executing command (showing partial info): mermaid-fixer --directory {} --llm-model {} --verbose",
            target_dir.display(),
        ...
        match cmd.status().await {
            Ok(status) => {
                if status.success() {
                    ...
                    println!(
                        "⚠️ mermaid-fixer execution completed but returned non-zero status code: {}",
                        status.code().unwrap_or(-1)
                    );
                    println!(
                        "💡 This may indicate some charts cannot be fixed, but won't affect subsequent processes"
                    );
                }
            }
            Err(e) => {
