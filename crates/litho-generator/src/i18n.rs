...
impl TargetLanguage {
    ...
    pub fn prompt_instruction(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Vietnamese => {
                "Hãy viết toàn bộ tài liệu bằng tiếng Việt tự nhiên, chính xác và dễ hiểu, sử dụng đúng thuật ngữ kỹ thuật."
            }
        }
    }
    ...
    pub fn get_directory_name(&self, dir_type: &str) -> String {
        match self {
            ...
            TargetLanguage::Vietnamese => match dir_type {
    ...
    pub fn get_doc_filename(&self, doc_type: &str) -> String {
        match self {
            ...
            TargetLanguage::Vietnamese => match doc_type {
    ...
    pub fn msg_config_read_error(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Japanese => {
                "⚠️ 警告: 設定ファイル {:?} を読み込めません、デフォルト設定を使用します"
            }
            TargetLanguage::Korean => {
                "⚠️ 경고: 설정 파일 {:?}을(를) 읽을 수 없습니다. 기본 설정을 사용합니다"
            }
            TargetLanguage::German => {
                "⚠️ Warnung: Konfigurationsdatei {:?} kann nicht gelesen werden, verwende Standardkonfiguration"
            }
            TargetLanguage::French => {
                "⚠️ Avertissement: Impossible de lire le fichier de configuration {:?}, utilisation de la configuration par défaut"
            }
            TargetLanguage::Russian => {
                "⚠️ Предупреждение: Не удается прочитать файл конфигурации {:?}, используется конфигурация по умолчанию"
            }
            TargetLanguage::Vietnamese => {
                "⚠️ Cảnh báo: Không thể đọc tệp cấu hình {:?}, sử dụng cấu hình mặc định"
            }
        }
    }
    ...
    pub fn msg_unknown_provider(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Japanese => {
                "⚠️ 警告: 不明なプロバイダー: {}、デフォルトプロバイダーを使用します"
            }
            TargetLanguage::Korean => {
                "⚠️ 경고: 알 수 없는 프로바이더: {}, 기본 프로바이더를 사용합니다"
            }
            TargetLanguage::German => {
                "⚠️ Warnung: Unbekannter Provider: {}, verwende Standard-Provider"
            }
            TargetLanguage::French => {
                "⚠️ Avertissement: Fournisseur inconnu: {}, utilisation du fournisseur par défaut"
            }
            TargetLanguage::Russian => {
                "⚠️ Предупреждение: Неизвестный провайдер: {}, используется провайдер по умолчанию"
            }
            TargetLanguage::Vietnamese => {
                "⚠️ Cảnh báo: Nhà cung cấp không xác định: {}, sử dụng nhà cung cấp mặc định"
            }
        }
    }
    ...
    pub fn msg_unknown_language(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Japanese => {
                "⚠️ 警告: 不明な対象言語: {}、デフォルト言語 (English) を使用します"
            }
            TargetLanguage::Korean => {
                "⚠️ 경고: 알 수 없는 대상 언어: {}, 기본 언어(English)를 사용합니다"
            }
            TargetLanguage::German => {
                "⚠️ Warnung: Unbekannte Zielsprache: {}, verwende Standardsprache (English)"
            }
            TargetLanguage::French => {
                "⚠️ Avertissement: Langue cible inconnue: {}, utilisation de la langue par défaut (English)"
            }
            TargetLanguage::Russian => {
                "⚠️ Предупреждение: Неизвестный целевой язык: {}, используется язык по умолчанию (English)"
            }
            TargetLanguage::Vietnamese => {
                "⚠️ Cảnh báo: Ngôn ngữ đích không xác định: {}, sử dụng ngôn ngữ mặc định (English)"
            }
        }
    }
    ...
    pub fn msg_cache_hit(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Russian => {
                "   ✅ Использование кэшированного результата AI-анализа: {}"
            }
    ...
    pub fn msg_ai_service_error(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Japanese => {
                "❌ モデルサービスの呼び出しに失敗しました。{} 回の試行に失敗...{}"
            }
            TargetLanguage::Korean => "❌ 모델 서비스 호출 오류, {} 번의 시도 모두 실패...{}",
            TargetLanguage::German => {
                "❌ Fehler beim Aufruf des Modell-Service, {} Versuche fehlgeschlagen...{}"
            }
            TargetLanguage::French => {
                "❌ Erreur d'appel du service de modèle, {} tentatives échouées...{}"
            }
            TargetLanguage::Russian => {
                "❌ Ошибка вызова модельного сервиса, {} попыток неудачно...{}"
            }
            TargetLanguage::Vietnamese => {
                "❌ Lỗi gọi dịch vụ mô hình, thất bại sau {} lần thử...{}"
            }
        }
    }
    ...
    pub fn msg_doc_not_found(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Russian => {
                "⚠️ Предупреждение: Содержимое документа не найдено, ключ: {}"
            }
    ...
    pub fn msg_mermaid_error(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::German => {
                "⚠️ Fehler während der Mermaid-Diagrammreparatur aufgetreten: {}"
            }
            TargetLanguage::French => {
                "⚠️ Erreur survenue lors de la réparation du diagramme mermaid: {}"
            }
            TargetLanguage::Russian => "⚠️ Ошибка при восстановлении диаграммы mermaid: {}",
            TargetLanguage::Vietnamese => {
                "⚠️ Lỗi xảy ra trong quá trình sửa chữa sơ đồ mermaid: {}"
            }
        }
    }
    ...
    pub fn msg_mermaid_not_installed(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::English => {
                "⚠️ Warning: mermaid-fixer not installed or unavailable, skipping mermaid diagram repair"
            }
            TargetLanguage::Japanese => {
                "⚠️ 警告: mermaid-fixer がインストールされていないか利用できません、mermaidダイアグラムの修復をスキップします"
            }
            TargetLanguage::Korean => {
                "⚠️ 경고: mermaid-fixer가 설치되지 않았거나 사용할 수 없습니다. mermaid 다이어그램 복구를 건너뜁니다"
            }
            TargetLanguage::German => {
                "⚠️ Warnung: mermaid-fixer nicht installiert oder nicht verfügbar, überspringe Mermaid-Diagrammreparatur"
            }
            TargetLanguage::French => {
                "⚠️ Avertissement: mermaid-fixer non installé ou indisponible, réparation du diagramme mermaid ignorée"
            }
            TargetLanguage::Russian => {
                "⚠️ Предупреждение: mermaid-fixer не установлен или недоступен, пропуск восстановления диаграммы mermaid"
            }
            TargetLanguage::Vietnamese => {
                "⚠️ Cảnh báo: mermaid-fixer chưa cài đặt hoặc không khả dụng, bỏ qua sửa chữa sơ đồ mermaid"
            }
        }
    }
    ...
    pub fn msg_summary_reasoning_failed(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::Japanese => {
                "⚠️  要約推論に失敗しました、元の部分的な結果を返します...{}"
            }
            TargetLanguage::Korean => "⚠️  요약 추론 실패, 원래 부분 결과 반환...{}",
            TargetLanguage::German => {
                "⚠️  Zusammenfassungs-Reasoning fehlgeschlagen, gebe ursprüngliches Teilergebnis zurück...{}"
            }
            TargetLanguage::French => {
                "⚠️  Échec du raisonnement de résumé, retour du résultat partiel d'origine...{}"
            }
            TargetLanguage::Russian => {
                "⚠️  Сбой суммирования, возврат исходного частичного результата...{}"
            }
            TargetLanguage::Vietnamese => {
                "⚠️  Suy luận tóm tắt thất bại, trả về kết quả một phần ban đầu...{}"
            }
        }
    }
    ...
    pub fn msg_max_iterations(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::French => {
                "   ⚠️ Nombre maximum d'itérations atteint ({}), interruption"
            }
            TargetLanguage::Russian => {
                "   ⚠️ Достигнуто максимальное количество итераций ({}), прерывание"
            }
    ...
    pub fn msg_domain_analysis_failed(&self) -> &'static str {
        match self {
            ...
            TargetLanguage::French => {
                "⚠️ Analyse du module de domaine: échec de l'analyse de {}: {}"
            }
