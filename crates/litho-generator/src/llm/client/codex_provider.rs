...
/// let client = CodexRsClient::new(Some("/usr/local/bin/codex"), Some(60)).unwrap();
/// ```
#[derive(Debug, Clone)]
...
impl CodexRsClient {
    ...
    pub fn new(binary_path: Option<&str>, timeout_secs: Option<u64>) -> Result<Self> {
    ...
    where
        T: DeserializeOwned,
    {
        let raw = self.prompt(system, user).await?;
...
mod tests {
    ...
    fn client_new_with_explicit_path() {
        ...
        let client = CodexRsClient::new(Some("/fake/path/codex"), Some(30));
        assert!(client.is_ok());
        let c = client.unwrap();
